...
export default new class extends render {
  ...
  async Webpage(url, {
    ...
    try {
      ...
      logger.error(`xunmiao-puppeteer 网页截图失败:${name}${err}`)
      /** 关闭浏览器 */
      if (this.browser) {
    ...
    if (!buff) {
      logger.error(`xunmiao-puppeteer 网页截图为空:${name}`)
      return false
    }
    ...
    logger.mark(
      `xunmiao-puppeteer[网页截图][${name}][${renderer.renderNum}次] ${kb} ${logger.green(
        `${Date.now() - start}ms`
      )}`
    )
    renderer.restart()
