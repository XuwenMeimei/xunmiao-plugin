...
export class tools extends plugin {
  ...
  async ping(e) {
    ...
    exec(command, async (err, stdout, stderr) => {
      ...
      msgList.push(`来自 ${targetDisplay} 的 Ping 检测结果\n`);
      msgList.push(`正在 Ping ${targetDisplay} 具有 32 字节的数据:`);

      timesAndTTL.forEach(({ time, ttl }) => {
        msgList.push(`来自 ${ip} 的回复: 字节=32 时间=${time}ms TTL=${ttl}`);
      });
      ...
      msgList.push('');
      msgList.push(`${ip} 的 Ping 统计信息:`);
      msgList.push(`    数据包: 已发送 = ${sent}，已接收 = ${received}，丢失 = ${lost} (${lossRate}% 丢失)，`);
      ...
      if (received > 0) {
        ...
        msgList.push(`往返行程的估计时间(以毫秒为单位):`);
        msgList.push(`    最短 = ${min}ms，最长 = ${max}ms，平均 = ${avg}ms`);
      }

      const forward = await Bot.makeForwardArray([
        msgList.join('\n')
      ]);
